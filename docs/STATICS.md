...
export function SessionHeader() {
  ...
  return (
    <div>
      <h1>{statics.track}</h1>
      <p>{statics.playerName} {statics.playerSurname}</p>
...
export function RaceEngineer() {
  ...
  return (
    <div>
      <h2>{statics.playerName} at {statics.track}</h2>
      <p>Speed: {speed?.toFixed(1)} km/h</p>
      <p>RPM: {rpm} / {maxRpm} ({rpmPercent.toFixed(0)}%)</p>
      <p>Fuel: {frame.data.fuel?.toFixed(1)}L / {statics.maxFuel}L</p>
    </div>
  )
}
```
