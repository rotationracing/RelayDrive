...
use crate::broadcast_protocol::protocol::acc_enum::{
...
fn driver(input: &[u8]) -> Res<&[u8], Driver> {
    context(
        "driver",
        tuple((
            le_u16,      // driver_index (ignored)
            boolean,     // has_driver_info
        )),
    )(input)
    .and_then(|(next_input, (_driver_index, has_info))| {
        if has_info {
            // Parse driver details if present
            context(
                "driver_details",
                tuple((
                    kstring,
                    kstring,
                    kstring, // nickname (ignored for now)
                    kstring,
                    map_res(le_u8, DriverCategory::try_from),
                )),
            )(next_input)
            .map(|(next_input, (first_name, last_name, _nickname, short_name, category))| {
                (
                    next_input,
                    Driver {
                        ...
                        category,
                        nationality: Nationality::Any, // Default when no info
                    },
                )
            })
        } else {
            // Return empty driver if no info
            Ok((
                next_input,
                Driver {
                    first_name: Cow::Borrowed(""),
...
fn entrylist_car(input: &[u8]) -> Res<&[u8], EntrylistCar> {
    context(
        ...
        tuple((
            ...
            kstring,
            le_i32,
            kstring, // TeamCarName (new field)
            kstring, // DisplayName (new field)
    ...
    .map(
        |(
            ...
            (
                ...
                team_name,
                race_number,
                _team_car_name, // Ignore for now
                _display_name,  // Ignore for now
                cup_category,
