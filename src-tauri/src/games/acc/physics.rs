pub const PHYSICS_EVENT: &str = "acc://physics";
...
mod platform {
    ...
    static PHYSICS_RUNNING: OnceLock<Mutex<bool>> = OnceLock::new();

    const READ_INTERVAL: Duration = Duration::from_micros(3_000); // ~333 Hz
    const EMIT_INTERVAL: Duration = Duration::from_millis(16); // ~60 FPS
    const MAX_IDLE_TICKS: u32 = 6_000;
    ...
    pub fn start_stream(app: AppHandle) -> Result<(), String> {
        let flag = PHYSICS_RUNNING.get_or_init(|| Mutex::new(false));
        let mut guard = flag
            .lock()
            .map_err(|_| "Failed to lock physics stream mutex".to_string())?;
        if *guard {
            return Ok(());
        }
        *guard = true;
        ...
        thread::spawn(move || {
            if let Err(err) = stream_loop(app_handle.clone()) {
                ...
                log::warn!("ACC physics stream stopped: {}", err);
            }

            if let Some(flag) = PHYSICS_RUNNING.get() {
    ...
    fn stream_loop(app: AppHandle) -> Result<(), String> {
        ...
        loop {
            match acc.read_shared_memory().map_err(acc_error_to_string)? {
                Some(data) => {
                    idle_ticks = 0;
                    if let Ok(value) = serde_json::to_value(&data.physics) {
                    ...
                    if last_emit.elapsed() >= EMIT_INTERVAL {
                        if let Some(ref data) = latest_data {
                            ...
                            app.emit(super::PHYSICS_EVENT, payload)
                                .map_err(|e| format!("Failed to emit physics payload: {e}"))?;
                        }
                    ...
                    if idle_ticks % 200 == 0 {
                        let _ = app.emit(
                            ...
                            PhysicsPayload {
                                ...
                                status: "waiting",
                                data: None,
                                message: Some("Awaiting physics data from ACC".into()),
                            },
                        );
                    }
...
pub fn start_stream(_: tauri::AppHandle) -> Result<(), String> {
    Err("ACC physics streaming is only available on Windows".into())
}
