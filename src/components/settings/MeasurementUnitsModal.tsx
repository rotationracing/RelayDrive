...
export function MeasurementUnitsModal({
  ...
  const Field = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="grid grid-cols-[minmax(0,1fr)_148px] items-center gap-4 border-b border-border py-3 last:border-b-0">
      ...
      <div className="w-[148px] justify-self-end">{children}</div>
    </div>
  );
      ...
      <DialogContent className="w-full max-w-lg gap-0 overflow-hidden rounded-[var(--radius-2xl)] border-border bg-card p-0 shadow-2xl">
        ...
        <div className="max-h-[60vh] overflow-y-auto px-6 py-0">
          <Field label={t("units.distance.label")}>
            <Select
              value={localChoice.distance}
              onValueChange={(v: DistanceChoice) => setLocalChoice((s) => ({ ...s, distance: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="metric">
                  {t("common.metric")} ({t("units.distance.metric_abbr")})
                </SelectItem>
                <SelectItem value="imperial">
                  {t("common.imperial")} ({t("units.distance.imperial_abbr")})
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
          ...
          <Field label={t("units.speed.label")}>
            <Select
              value={localChoice.speed}
              onValueChange={(v: SpeedChoice) => setLocalChoice((s) => ({ ...s, speed: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="metric">
                  {t("common.metric")} ({t("units.speed.metric_abbr")})
                </SelectItem>
                <SelectItem value="imperial">
                  {t("common.imperial")} ({t("units.speed.imperial_abbr")})
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
          ...
          <Field label={t("units.acceleration.label")}>
            <Select
              value={localChoice.acceleration}
              onValueChange={(v: AccelChoice) => setLocalChoice((s) => ({ ...s, acceleration: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="metric">
                  {t("common.metric")} ({t("units.acceleration.metric_abbr")})
                </SelectItem>
                <SelectItem value="imperial">
                  {t("common.imperial")} ({t("units.acceleration.imperial_abbr")})
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
          ...
          <Field label={t("units.temperature.label")}>
            <Select
              value={localChoice.temperature}
              onValueChange={(v: TemperatureChoice) =>
                setLocalChoice((s) => ({ ...s, temperature: v }))
              }
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
          ...
          <Field label={t("units.pressure.label")}>
            <Select
              value={localChoice.pressure}
              onValueChange={(v: PressureChoice) => setLocalChoice((s) => ({ ...s, pressure: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
          ...
          <Field label={t("units.torque.label")}>
            <Select
              value={localChoice.torque}
              onValueChange={(v: TorqueChoice) => setLocalChoice((s) => ({ ...s, torque: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
          ...
          <Field label={t("units.power.label")}>
            <Select
              value={localChoice.power}
              onValueChange={(v: PowerChoice) => setLocalChoice((s) => ({ ...s, power: v }))}
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              </SelectTrigger>
          ...
          <Field label={t("units.fuel_volume.label")}>
            <Select
              value={localChoice.fuel_volume}
              onValueChange={(v: FuelVolumeChoice) =>
                setLocalChoice((s) => ({ ...s, fuel_volume: v }))
              }
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              ...
              <SelectContent>
                <SelectItem value="metric">
                  {t("common.metric")} ({t("units.fuel_volume.metric_abbr")})
                </SelectItem>
                <SelectItem value="imperial">
                  {t("common.imperial")} ({t("units.fuel_volume.imperial_abbr")})
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
          ...
          <Field label={t("units.suspension_travel.label")}>
            <Select
              value={localChoice.suspension_travel}
              onValueChange={(v: SuspensionTravelChoice) =>
                setLocalChoice((s) => ({ ...s, suspension_travel: v }))
              }
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
          ...
          <Field label={t("units.tire_pressure.label")}>
            <Select
              value={localChoice.tire_pressure}
              onValueChange={(v: TirePressureChoice) =>
                setLocalChoice((s) => ({ ...s, tire_pressure: v }))
              }
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
          ...
          <Field label={t("units.fuel_consumption.label")}>
            <Select
              value={localChoice.fuel_consumption}
              onValueChange={(v: FuelConsumptionChoice) =>
                setLocalChoice((s) => ({ ...s, fuel_consumption: v }))
              }
            >
              <SelectTrigger className="h-11 w-full rounded-[var(--radius-lg)] border-border bg-input px-3">
                <SelectValue />
              ...
              <SelectContent>
                <SelectItem value="metric">
                  {t("common.metric")} ({t("units.fuel_consumption.metric_abbr")})
                </SelectItem>
                <SelectItem value="imperial">
                  {t("common.imperial")} ({t("units.fuel_consumption.imperial_abbr")})
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
        ...
        <div className="flex justify-end gap-2 border-t border-border px-6 py-4">
          ...
          <Button onClick={handleSave} className="rounded-[var(--radius-lg)]">
            {t("common.save")}
          </Button>
        </div>
      </DialogContent>
