...
import { registerGlobalShortcut, unregisterGlobalShortcut, unregisterAllGlobalShortcuts } from "@/app/tauri-bridge"
...
export function GlobalHotkeyListener() {
  ...
  useEffect(() => {
    ...
    console.log("[GlobalHotkeyListener] Using hotkeys - edit mode:", editModeHotkey, "overlays enabled:", overlaysEnabledHotkey)
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (matchesHotkey(event, editModeHotkey)) {
        ...
        if (now - lastTime < 200) {
          console.log("[GlobalHotkeyListener] Ignoring rapid toggle from document listener (debounced)")
          return
        }
        lastToggleTimeRef.current.set("toggle_overlay_edit_mode", now)
        console.log("[GlobalHotkeyListener] Hotkey matched (document listener). Triggering edit mode action")
      ...
      if (matchesHotkey(event, overlaysEnabledHotkey)) {
        ...
        // Debounce: prevent toggling if less than 200ms since last toggle
        if (now - lastTime < 200) {
          console.log("[GlobalHotkeyListener] Ignoring rapid overlays toggle from document listener (debounced)")
          return
        }
        lastToggleTimeRef.current.set("toggle_overlays_enabled", now)
        console.log("[GlobalHotkeyListener] Hotkey matched (document listener). Triggering overlays enabled action")
        event.preventDefault()
        event.stopPropagation()
    ...
    if (!isTauri) {
      console.warn("[GlobalHotkeyListener] Tauri API unavailable; global shortcut disabled. Run via 'bun run tauri:dev'.")
      return () => {
    ...
    console.log("[GlobalHotkeyListener] Tauri shortcuts - edit mode:", editModeTauriShortcut, "overlays enabled:", overlaysEnabledTauriShortcut)
        ...
        try {
          const unlisten = await listen<string>("global-shortcut://triggered", (event) => {
            ...
            if (event.payload === editModeTauriShortcut) {
              ...
              lastToggleTimeRef.current.set("toggle_overlay_edit_mode", now)
              console.log("[GlobalHotkeyListener] Global shortcut matched. Triggering edit mode action")
              triggerAction("toggle_overlay_edit_mode")
              ...
              lastToggleTimeRef.current.set("toggle_overlays_enabled", now)
              console.log("[GlobalHotkeyListener] Global shortcut matched. Triggering overlays enabled action")
              triggerAction("toggle_overlays_enabled")
            }
          })
          ...
          console.error("[GlobalHotkeyListener] Failed to set up global shortcut listener:", listenErr)
        }
      }

      const registerShortcut = async (shortcut: string | null, name: string, attempt = 0): Promise<void> => {
        if (!shortcut) {
          console.warn(`[GlobalHotkeyListener] Unable to convert ${name} hotkey for Tauri global shortcut`)
          return
        }

        try {
          console.log(`[GlobalHotkeyListener] Registering global shortcut (attempt ${attempt + 1}):`, shortcut, `(${name})`)
          ...
          const isAlreadyRegistered = message.includes("already registered") || message.includes("HotKey already registered")
          
          if (attempt === 0 && isAlreadyRegistered) {
            console.warn(`[GlobalHotkeyListener] ${name} shortcut already registered. Retrying after clearing.`)
            try {
              ...
              console.warn(`[GlobalHotkeyListener] Failed to clear shortcuts before retry:`, clearErr)
            }
            // Retry once
  ...
  }, [settings?.hotkeys?.toggle_overlay_edit_mode, settings?.hotkeys?.toggle_overlays_enabled, triggerAction])
