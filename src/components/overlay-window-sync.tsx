...
export function OverlayWindowSync() {
      ...
      try {
        ...
        const loadedOverlays: OverlayInstance[] = overlayModulesRef.current.map((module, index) => {
          ...
          return {
            ...
            enabled:
              overlayEnabledStates[module.id] ??
              configItem?.enabled ??
              module.defaultEnabled ??
              false,
            position: configItem?.position
              ? { x: configItem.position.x, y: configItem.position.y }
              : (module.defaultPosition ?? fallbackPosition),
            size: configItem?.size ?? module.defaultSize ?? 100,
            opacity: configItem?.opacity ?? module.defaultOpacity ?? 100,
            componentSettings:
              configItem?.componentSettings ??
              (() => {
        ...
        console.log(
          "[OverlayWindowSync] Loaded overlay config:",
          loadedOverlays.length,
          "overlays",
        );
      } catch (error) {
  ...
  useEffect(() => {
      ...
      if (!isInitializedRef.current) {
        const loadConfig = async () => {
          try {
            ...
            // Convert config items to overlay instances
            // config.overlays is a Record<string, OverlayConfigItem>, not an array
            const loadedOverlays: OverlayInstance[] = overlayModulesRef.current.map(
              (module, index) => {
                const configItem = config.overlays?.[module.id];
                const fallbackPosition = { x: 80 + index * 80, y: 80 + index * 60 };
            ...
            isInitializedRef.current = true;
            console.log(
              "[OverlayWindowSync] Loaded overlay config on mount:",
              loadedOverlays.length,
              "overlays",
            );
          } catch (error) {
    ...
    if (prevOverlaysEnabledRef.current !== overlaysEnabled) {
      console.log(
        `[OverlayWindowSync] overlaysEnabled changed from ${prevOverlaysEnabledRef.current} to ${overlaysEnabled}. Syncing windows.`,
      );
