...
export function useOverlayManagement({ overlayModules, saveDefaultConfig }: UseOverlayManagementOptions) {
  const moduleMap = useMemo(() => {
    const entries = overlayModules.map((module) => [module.id, module] as const)
    return new Map(entries)
  }, [overlayModules])

  const { overlaysEnabled, setOverlaysEnabled, overlayEnabledStates, setOverlayEnabled, initializeOverlayStates, editMode, toggleEditMode } = useOverlaySessionStore()
  ...
  useEffect(() => {
    ...
    if (prevOverlaysEnabledRef.current !== overlaysEnabled) {
      console.log(`[useOverlayManagement] Store overlaysEnabled changed externally from ${prevOverlaysEnabledRef.current} to ${overlaysEnabled}. Syncing windows.`)
      ...
      Promise.all(
        overlays.map(async (overlay) => {
          try {
            ...
            console.warn(`[useOverlayManagement] Failed to capture position for ${overlay.id}:`, error)
          }
          return overlay
        }),
      )
        .then((capturedPositions) => {
          ...
          updatedOverlays.forEach((overlay) => {
            ...
            if (overlaysEnabled && overlay.enabled) {
              console.log(`[useOverlayManagement] Showing overlay ${overlay.id} (synced from store)`)
              void showOverlayWindow(overlay, module)
            } else if (!overlaysEnabled && overlay.enabled) {
              console.log(`[useOverlayManagement] Hiding overlay ${overlay.id} (synced from store)`)
  ...
  useEffect(() => {
    ...
    void registerOverlayPositionListener(({ id, position }) => {
      setOverlays((prev) => {
        const updated = prev.map((overlay) => (overlay.id === id ? { ...overlay, position } : overlay))
        if (saveDefaultConfig) {
    ...
    void registerOverlayClosedListener(({ id }) => {
      setOverlays((prev) => {
        const updated = prev.map((overlay) => (overlay.id === id ? { ...overlay, enabled: false } : overlay))
        if (saveDefaultConfig) {
          void saveDefaultConfig(updated)
        }
        return updated
      ...
      setOverlays((prev) => {
          ...
          try {
            ...
            if (win) {
              ...
              win.outerPosition().then((pos) => {
                if (pos && Number.isFinite(pos.x) && Number.isFinite(pos.y)) {
                  setOverlays((p) =>
                    p.map((o) => (o.id === id ? { ...o, position: { x: Number(pos.x), y: Number(pos.y) } } : o)),
                  )
                }
              }).catch(() => {})
            }
        ...
        const updated = prev.map((overlay) => {
          ...
          console.log(`[toggleOverlay] Updated overlay ${id}, enabled=${updatedOverlay.enabled}`, {
            ...
            hasComponentSettings: !!updatedOverlay.componentSettings && Object.keys(updatedOverlay.componentSettings).length > 0,
          })
  ...
  const debouncedSave = useCallback((overlays: OverlayInstance[], delay: number = 1000) => {
      ...
      setOverlays((prev) => {
        ...
        Promise.all(
          ...
          try {
            ...
            console.warn(`[toggleOverlaysEnabled] Failed to capture position for ${overlay.id}:`, error)
          }
          return overlay
            }),
          )
            .then((capturedPositions) => {
              ...
              console.log(`[toggleOverlaysEnabled] Showing/hiding overlays with componentSettings:`, 
                updatedOverlaysAfterCapture.map(o => ({ id: o.id, hasSettings: !!o.componentSettings && Object.keys(o.componentSettings).length > 0 }))
              )
