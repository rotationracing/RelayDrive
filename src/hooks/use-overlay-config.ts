...
export function useOverlayConfig({
    ...
    const enriched = await Promise.all(
      snapshot.map(async (overlay) => {
        ...
        try {
          ...
          console.warn(
            `[captureOverlayPositions] Failed to capture position for ${overlay.id}:`,
            error,
          );
        }
  ...
  const saveDefaultConfig = useCallback(
    async (updatedOverlays: OverlayInstance[]) => {
      try {
        ...
        console.log(
          `[saveDefaultConfig] Saving ${Object.keys(overlayStates).length} overlays to default.json`,
        );
        await saveOverlayConfig("default", { overlays: overlayStates });
        console.log(
          `[saveDefaultConfig] Successfully saved ${Object.keys(overlayStates).length} overlays to default.json`,
        );
      } catch (error) {
        if (
        ...
        void listOverlayConfigs()
          ...
          .catch((err) => {
            if (
              ...
              (err.message.includes("invoke") || err.message.includes("Tauri"))
            ) {
              return;
            }
    ...
    const loadDefault = async () => {
      ...
      try {
        const config = await loadOverlayConfig(
          presetToLoad === "default" ? undefined : presetToLoad,
        );
        ...
        if (!hasSavedOverlays && presetToLoad !== "default") {
          ...
          if (
            ...
            Object.keys(defaultConfig.overlays).length > 0
          ) {
        ...
        if (hasSavedOverlays) {
          console.log(
            "[overlay] Loading saved overlay states from JSON:",
            Object.keys(config.overlays),
          );
          ...
          console.log(
            "[overlay] Attempting to save default.json with",
            Object.keys(overlayStates).length,
            "overlays",
          );
          saveOverlayConfig("default", { overlays: overlayStates })
            .then(() => {
              console.log(
                "[overlay] ✓ Created and populated default.json with",
                Object.keys(overlayStates).length,
                "overlays",
              );
              loadingDefaultRef.current = false;
  ...
  useEffect(() => {
    if (loadingFromFileRef.current || selectedPreset === "default" || loadingDefaultRef.current)
      return;

    loadingFromFileRef.current = true;

    void loadOverlayConfig(selectedPreset)
      .then((config) => {
              ...
              if (saved?.componentSettings) {
                Object.entries(saved.componentSettings).forEach(([key, value]) => {
                  if (key in componentSettings) {
                    componentSettings[key] = value as
                      | number
                      | string
                      | boolean
