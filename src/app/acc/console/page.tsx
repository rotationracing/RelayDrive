...
const HIDDEN_MESSAGES = [
  "Shared memory not available or game not running",
  "Awaiting physics data from ACC",
];
  ...
  const metrics = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Brake",
        value:
          latest?.data?.brake !== undefined ? `${(latest.data.brake * 100).toFixed(1)}%` : "--",
        helper: "Brake pedal",
        icon: Power,
      },
      {
        ...
        label: "Steering",
        value:
          latest?.data?.steer_angle !== undefined ? `${latest.data.steer_angle.toFixed(1)}°` : "--",
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Live ACC telemetry and broadcast data streamed directly into RelayDrive.
          </p>
        </div>
        ...
        <Card className="rounded-[var(--radius-xl)]">
          <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>
                Translated ACC shared memory events (~60 FPS) and broadcast data (~10 Hz).
              </CardDescription>
            </div>
                ...
                <div className="space-y-3 font-mono text-xs text-slate-100">
                  {lines.map((line) => (
                    ...
                    >
                      ...
                      <span className={`font-semibold ${lineToneClass(line.status)}`}>
...
function createBroadcastConsoleLine(frame: any): ConsoleLine | null {
  ...
  if (data.RegistrationResult) {
    ...
    const lapTime = car.current_lap?.lap_time_ms
      ? (car.current_lap.lap_time_ms / 1000).toFixed(3)
      : "N/A";
