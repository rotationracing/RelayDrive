...
import { closeOverlayWindow as invokeCloseOverlayWindow, createOverlayWindow as invokeCreateOverlayWindow, updateOverlayWindowState as invokeUpdateOverlayWindowState } from "@/app/tauri-bridge"
...
const getWindowSize = (overlay: OverlayInstance, module: OverlayModule) => {
  ...
  if (overlay.id === "delta") {
    ...
    if (textPosition === "above" || textPosition === "under") {
      ...
      height += textHeight
      console.log(`[getWindowSize] Delta overlay: textPosition=${textPosition}, fontSize=${fontSize}, textLineHeight=${textLineHeight}, adding ${textHeight}px height, total=${height}`)
    }
  }
...
const ensureOverlayWindowHandle = async (overlay: OverlayInstance, module: OverlayModule) => {
  ...
  if (creatingWindows.has(overlay.id)) {
    console.log(`[ensureOverlayWindowHandle] Window creation already in progress for ${overlay.id}, waiting...`)
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      if (win) {
        console.log(`[ensureOverlayWindowHandle] Found window after waiting for concurrent creation`)
  ...
  try {
    ...
    try {
      ...
      console.warn(`[ensureOverlayWindowHandle] Backend creation failed, checking if window exists:`, error)
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      win = (await WebviewWindow.getByLabel(label)) ?? undefined
      if (win) {
        console.log(`[ensureOverlayWindowHandle] Got window from label ${label} after ${attempt + 1} attempts`)
        break
      }
    }
    ...
    if (!win) {
      ...
      console.warn(`[ensureOverlayWindowHandle] Backend created window but we can't find it by label ${label}`)
    ...
    if (!win) {
      console.error(`[ensureOverlayWindowHandle] Failed to create or find window for ${overlay.id}`)
      return undefined
    }
    ...
    overlayWindows.set(overlay.id, win)
    console.log(`[ensureOverlayWindowHandle] Tracking window ${overlay.id}, total tracked: ${overlayWindows.size}`)
    ensureReady(win, overlay.id)
...
export const showOverlayWindow = async (overlay: OverlayInstance, module: OverlayModule) => {
  ...
  try {
    ...
    const waitForReady = () => new Promise<void>((resolve) => {
      ...
      if (mainWindow) {
        mainWindow.listen("overlay://ready", (event) => {
          try {
    ...
    console.log(`[showOverlayWindow] Emitting state to ${getOverlayLabel(overlay.id)}`, {
      ...
      componentSettingsKeys: overlay.componentSettings ? Object.keys(overlay.componentSettings) : 'none',
      size: overlay.size,
...
export const updateOverlayWindow = async (overlay: OverlayInstance, module: OverlayModule, options?: { updatePosition?: boolean }) => {
...
export const broadcastMoveMode = async (enabled: boolean) => {
  ...
  console.log(`[broadcastMoveMode] Broadcasting move mode: ${enabled} to ${overlayWindows.size} windows`)
...
export const registerOverlayPositionListener = (handler: (payload: { id: string; position: { x: number; y: number } }) => void) => {
