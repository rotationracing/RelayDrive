...
export default function Page() {
  ...
  const { initializeOverlayStates, setOverlaysEnabled, setOverlayEnabled, overlaysEnabled: storeOverlaysEnabled } = useOverlaySessionStore()
  ...
  const { overlays, setOverlays, moduleMap, overlaysEnabled, editMode, toggleEditMode, toggleOverlay, updateOverlay, toggleOverlaysEnabled } = managementHook
  ...
  }, [editMode, overlays, configHook.selectedPreset, configHook.saveDefaultConfig, configHook.savePresetConfig])
  ...
  useEffect(() => {
    ...
    if (prevStoreOverlaysEnabledRef.current !== storeOverlaysEnabled) {
      console.log(`[Page] Store overlaysEnabled changed externally from ${prevStoreOverlaysEnabledRef.current} to ${storeOverlaysEnabled}. Syncing windows.`)
    ...
    if (configHook.selectedPreset && configHook.selectedPreset !== "default") {
      try {
        const capturedPositions = await Promise.all(
          overlays.map(async (overlay) => {
            try {
              const win = await import("./overlay-window-manager").then((m) => m.getOverlayWindow(overlay.id))
              if (win) {
                const pos = await win.outerPosition()
                if (pos && Number.isFinite(pos.x) && Number.isFinite(pos.y)) {
                  return { ...overlay, position: { x: Number(pos.x), y: Number(pos.y) } }
  ...
  const activeOverlay = useMemo(() => overlays.find((o) => o.id === activeId) ?? null, [activeId, overlays])
