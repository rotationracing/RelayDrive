...
function OverlayWindowContent() {
  ...
  const overlayModule = useMemo(() => (overlayId ? getOverlayModule(overlayId) : undefined), [overlayId])
  ...
  useEffect(() => {
    ...
    console.log(`[overlay-${overlayId}] Setting up event listeners, isTauri=${isTauri()}, window.__TAURI__=${(window as any).__TAURI__}, window.__TAURI_IPC__=${(window as any).__TAURI_IPC__}`)
    ...
    listen("overlay://state", (event) => {
      ...
      const hasSettings = receivedSettings !== undefined && receivedSettings !== null && typeof receivedSettings === 'object'
      ...
      setState((prev) => {
        ...
        const newComponentSettings = hasSettings
          ? receivedSettings
  ...
  const handleMouseDown = (event: React.MouseEvent) => {
    console.log(`[overlay-${overlayId}] Mouse down, moveMode=${state.moveMode}, button=${event.button}, currentWindow=${!!currentWindow}`)
    if (!state.moveMode) {
  ...
  console.log(`[overlay-${overlayId}] Rendering, moveMode=${state.moveMode}, will show border: ${state.moveMode}`)
