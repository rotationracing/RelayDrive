...
/**
 * Convert a raw ACC car model string into a human-friendly label.
 * Falls back to a simple formatter when we don't have an explicit map entry.
 */
export function prettifyCarName(model: string): string {
  if (!model) return "--"
  const key = model.toLowerCase()
  const mapped = carNameMap[key]
  if (mapped) return mapped

  const words = key.split("_").map((token) => {
    if (SPECIAL_TOKENS[token]) return SPECIAL_TOKENS[token]
