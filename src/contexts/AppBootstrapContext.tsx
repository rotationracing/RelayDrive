...
export function AppBootstrapProvider({ children }: { children: React.ReactNode }) {
  ...
  const ensureUpdateCheck = useCallback(async (force = false) => {
    if (!settings) return
  ...
  const value = useMemo(
    ...
    [updateInfo, checkingUpdate, checkedAt, ensureUpdateCheck, refreshUpdateCheck, isLaunching, launchingLabel, appVersion],
  )
